...
START_POSE = np.array([-1.5, -1.5, 0.0], dtype=np.float32)
MAX_ITERATIONS = 500
WALL_OFFSET = 2.0
...
class Node(object):
    ...
    def direction(self):
        return np.array(
            [np.cos(self._pose[YAW]), np.sin(self._pose[YAW])], dtype=np.float32
        )
...
def rrt(start_pose, goal_position, occupancy_grid):
    ...
    for _ in range(MAX_ITERATIONS):
        ...
        potential_parent = sorted(
            ((n, np.linalg.norm(position - n.position)) for n in graph),
            key=lambda x: x[1],
        )
        ...
        for n, d in potential_parent:
            if (
                d > 0.2
                and d < 1.5
                and n.direction.dot(position - n.position) / d > 0.70710678118
            ):
                u = n
                break
        else:
        ...
        v.parent = u
        graph.append(v)
        if np.linalg.norm(v.position - goal_position) < 0.2:
            final_node = v
            break
    return start_node, final_node
...
def find_circle(node_a, node_b):
    ...
    if np.abs(t) < 1e-3:
        ...
        center = (node_b.position + node_a.position) / 2.0
...
def draw_solution(start_node, final_node=None):
    ...
    def draw_path(u, v, arrow_length=0.1, color=(0.8, 0.8, 0.8), lw=1):
        du = u.direction
        plt.arrow(
            u.pose[X],
            u.pose[Y],
        ...
        if np.cross(u.direction, du).item() > 0.0:
    ...
    plt.scatter(points[:, 0], points[:, 1], s=10, marker="o", color=(0.8, 0.8, 0.8))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--map", action="store", default="map", help="Which map to use."
    )
    ...
    fig, ax = plt.subplots()
    occupancy_grid.draw()
    plt.scatter(0.3, 0.2, s=10, marker="o", color="green", zorder=1000)
    draw_solution(start_node, final_node)
    plt.scatter(
        START_POSE[0], START_POSE[1], s=10, marker="o", color="green", zorder=1000
    )
    plt.scatter(
        GOAL_POSITION[0], GOAL_POSITION[1], s=10, marker="o", color="red", zorder=1000
    )

    plt.axis("equal")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.xlim([-0.5 - 2.0, 2.0 + 0.5])
    plt.ylim([-0.5 - 2.0, 2.0 + 0.5])
    plt.show()
