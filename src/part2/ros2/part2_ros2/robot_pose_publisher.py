...
The fake_localization node uses this to compute map->odom TF.
"""
...
class RobotPosePublisher:
    def init(self, webots_node, properties):
        ...
        # Create publisher for absolute pose (ground truth)
        self.__pose_pub = self.__node.create_publisher(PoseStamped, "/robot_pose", 10)

        self.__node.get_logger().info(
            "RobotPosePublisher initialized, publishing ground truth to /robot_pose"
        )
